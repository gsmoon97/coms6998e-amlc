...
# --- NVIDIA T4 Hardware Specs ---
PEAK_COMPUTE = 8100  # GFLOPs/sec (FP32)
PEAK_BANDWIDTH = 320 # GB/sec
RIDGE_POINT = PEAK_COMPUTE / PEAK_BANDWIDTH

# The 6 files generated for the T4 experiments
csv_files = [
    'resnet50_train_t4.csv', 'resnet50_eval_t4.csv',
    'vgg16_train_t4.csv', 'vgg16_eval_t4.csv',
    'vit_train_t4.csv', 'vit_eval_t4.csv'
]

# Map filenames to pretty plot labels for the legend
label_map = {
    'resnet50_train_t4.csv': 'ResNet-50 (Train)',
    'resnet50_eval_t4.csv': 'ResNet-50 (Eval)',
    'vgg16_train_t4.csv': 'VGG-16 (Train)',
    'vgg16_eval_t4.csv': 'VGG-16 (Eval)',
    'vit_train_t4.csv': 'ViT-B-16 (Train)',
    'vit_eval_t4.csv': 'ViT-B-16 (Eval)'
}
...
if models_ai and any(perf > 0 for perf in models_perf.values()):
    ...
    ax.plot(ai_vals, perf_vals, color='black', linewidth=2, label='T4 Theoretical Roofline')
    ...
    ax.set_title('Empirical Roofline Model: Architectures on NVIDIA T4', fontsize=14)
    ...
    plt.savefig('t4_roofline_empirical.png', dpi=300, bbox_inches='tight')
    print("Roofline plot saved as 't4_roofline_empirical.png'")
else:
